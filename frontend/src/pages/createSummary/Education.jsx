      ...
      <div className="flex flex-col">
        ...
        <div className="ml-[100px] mt-[30px]">
          ...
          <form className="flex flex-col">
            {educations.map((edu) => (
              <label htmlFor={edu.name} key={edu.id} className="text-[20px] my-1 flex items-center gap-2">
                <input type="radio" id={edu.name} name="educaition" className="w-5 h-5" />
                <span>{edu.name}</span>
              </label>
            ))}
          </form>
          <div className="flex justify-between mt-10 w-full border-t border-gray-400 pt-5">
          <Link to="/works/detail" className="rounded-lg border border-gray-400 px-3 py-2">
            Go back
          </Link>
          <Link to="/works/educationinfo" className="rounded-lg border border-gray-400 px-3 py-2">
            Save and Continue
          </Link>
        </div>
        </div>
